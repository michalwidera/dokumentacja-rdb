...
`extractIntermediateStreams` wyodrębnia tutaj substrat `STREAM_ADD_core0_core1` dla wyrażenia `core0+core1`. Artefakt `str4` zależy od niego (Rys. 35):

<figure><img src="../assets/absorb_bez_mysum.svg" alt=""><figcaption><p>Rys. 35. Graf z automatycznym substratem STREAM_ADD_core0_core1</p></figcaption></figure>
...
substrat `STREAM_ADD_core0_core1` spełnia wszystkie warunki równoważności względem `mysum` — identyczny interwał, identyczny program tokenów, identyczny schemat pól. Faza `deduplicateSubstrats()` usuwa substrat i przepina wszystkie odwołania `PUSH_STREAM` na `mysum`. Substrat znika z grafu w zupełności (Rys. 36):

<figure><img src="../assets/absorb_z_mysum.svg" alt=""><figcaption><p>Rys. 36. Graf po dodaniu SELECT * STREAM mysum FROM core0+core1 — substrat zastąpiony przez jawny strumień</p></figcaption></figure>
